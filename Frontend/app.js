...
let signer;
const contractAddress = '0x4a6FD39Ad03B1AFF87Cf8071a8421B68Cf854c08';
const contractABI = [
	{
		"inputs": [
			{
...
class ResetPeriodObserver extends  IContractEventObserver {
	update(eventResult){
		console.log('Period Reset', eventResult);
		displayProposals();
		updateVideoFromLastWinningProposal();
	}
}
...
class RemovedProposelObserver extends  IContractEventObserver {
	update(eventResult){
...
const removedProposelObserver = new RemovedProposelObserver();
const resetPeriodObserver = new ResetPeriodObserver();
...
const listenerRemovedProposelObserver = new ContractEventListener(contract);
const listenerResetPeriodObserver = new ResetPeriodListener(contract);
...
listenerRemovedProposelObserver.subscribe(removedProposelObserver);
listenerRemovedProposelObserver.listenToEvent('RemovedProposel');

listenerResetPeriodObserver.subscribe(resetPeriodObserver);
listenerResetPeriodObserver.listenToEvent('ResetPeriod');
...
async function estimateGasFee(proposalId) {
	try {
        	const contract = new ethers.Contract(contractAddress, contractABI, signer);
	...
	try {
		...
		proposals.forEach(proposal => {
			...
			">
				...
				<button onClick="estimateGasFee(${proposal.id})">Estimate Gas Fee</button>
				</div>
            `;
...
function startPollingEthPrice() {
    ...
    fetchAndUpdateEthPrice();
    console.log("Setting interval...");
    setInterval(fetchAndUpdateEthPrice, 2000);
    console.error("AAAAA"); // This will log immediately after setting the interval
}
